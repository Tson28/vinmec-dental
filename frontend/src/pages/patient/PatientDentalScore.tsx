    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-24 h-24">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`text-xl font-display font-bold ${textColor}`}>{value}</span>
        </div>
      </div>
      <p className="text-xs font-semibold text-surface-600 text-center">{label}</p>
    </div>
  )
}
...
export default function PatientDentalScore() {
  ...
  const overallLabel = overall >= 80 ? 'Excellent 🌟' : overall >= 60 ? 'Good 👍' : 'Needs Attention ⚠️'
  ...
  const radarOptions = {
    responsive: true,
    scales: { r: { min: 0, max: 100, ticks: { stepSize: 25 }, grid: { color: '#e2e8f0' } } },
    plugins: { legend: { display: false } },
  }
  ...
  const tips = [
    { icon: '🪥', title: 'Brushing', score: score.cleanliness, advice: score.cleanliness < 75 ? 'Try brushing for a full 2 minutes, twice daily.' : 'Great brushing habits! Keep it up.' },
    { icon: '🧵', title: 'Flossing', score: score.gumHealth, advice: score.gumHealth < 80 ? 'Floss daily to reduce gum inflammation.' : 'Your gums are in great shape!' },
    { icon: '🍎', title: 'Diet', score: score.toothDecay, advice: score.toothDecay < 70 ? 'Reduce sugary drinks and snacks to protect enamel.' : 'Your diet is supporting good dental health.' },
  ]
    ...
    <DashboardLayout title="Dental Score">
      <div className="space-y-6">
        ...
        <div className="card bg-gradient-dental text-white border-0 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div>
            <p className="text-dental-100 text-sm font-medium">Overall Dental Health</p>
            <h2 className="font-display font-bold text-3xl mt-1">{overallLabel}</h2>
            <p className="text-dental-200 text-sm mt-2 max-w-md">
              Your dental health score is calculated based on gum health, tooth decay risk, alignment, and cleanliness metrics from your recent checkups.
            </p>
          </div>
        </div>
        ...
        {/* Progress Bars */}
        <div className="card">
          <h3 className="font-bold text-surface-800 mb-4">Detailed Metrics</h3>
          <div className="space-y-4">
            {metrics.map(m => (
              <div key={m.label}>
                <div className="flex justify-between mb-1.5">
                  <span className="text-sm font-semibold text-surface-700">{m.label}</span>
                  <span className="text-sm font-bold" style={{ color: m.color }}>{m.value}/100</span>
                </div>
        ...
        {/* Tips */}
        <div className="card">
          <h3 className="font-bold text-surface-800 mb-4">Personalized Recommendations</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {tips.map(t => (
              <div key={t.title} className={`p-4 rounded-xl border ${t.score < 75 ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200'}`}>
                <p className="text-2xl mb-2">{t.icon}</p>
                <p className="font-bold text-surface-800 text-sm">{t.title}</p>
        ...
        <div className="card bg-gradient-to-br from-dental-50 to-mint-50 border-dental-200 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h3 className="font-bold text-surface-800">Schedule Your Next Checkup</h3>
            <p className="text-sm text-surface-500 mt-1">Regular visits help maintain and improve your dental score</p>
          </div>
          <a href="/patient/appointments" className="btn-primary">Book Appointment →</a>
        </div>
      </div>
    </DashboardLayout>
  )
