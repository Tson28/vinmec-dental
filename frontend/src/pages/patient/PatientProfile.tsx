...
export default function PatientProfile() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      if (user?.id) await patientApi.update(user.id, form)
      setSuccess(true)
      setEditing(false)
  ...
  return (
    <DashboardLayout title="My Profile">
      <div className="max-w-2xl space-y-6">
        {success && <div className="px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-sm text-emerald-600">✅ Profile updated successfully!</div>}
        ...
        <div className="card flex items-center gap-6">
          ...
          <div>
            <h2 className="font-display font-bold text-2xl text-surface-900">{user?.name}</h2>
            <span className="badge badge-green mt-1">Patient</span>
            <p className="text-sm text-surface-500 mt-1">{user?.email}</p>
          </div>
          <button onClick={() => setEditing(!editing)} className="ml-auto btn-secondary">
        ...
        {/* Info */}
        <div className="card">
          <h3 className="font-bold text-surface-800 mb-4">Personal Information</h3>
          {editing ? (
