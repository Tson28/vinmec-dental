...
export default function DoctorImages() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { await imageApi.upload(fd); refetch() } finally { setUploading(false) }
  }
    ...
    <DashboardLayout title="Images Gallery">
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="font-display font-bold text-xl text-surface-900">All Patient Images</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map(img => (
              <div key={img.id} onClick={() => setPreview(img)}
                ...
                <img src={img.url} alt={img.description || 'Dental image'} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                <div className="absolute inset-0 bg-gradient-to-t from-surface-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-3">
                  <p className="text-white text-xs font-semibold">{img.patientName}</p>
                  <p className="text-white/70 text-[10px]">{typeIcon[img.type]} {img.type} • {img.uploadedAt?.split('T')[0]}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Preview */}
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface-900/80 backdrop-blur-sm p-4" onClick={() => setPreview(null)}>
          <div className="relative max-w-3xl w-full bg-white rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <img src={preview.url} alt={preview.description} className="w-full max-h-[70vh] object-contain bg-surface-900" />
            <div className="p-4 flex items-center justify-between">
              <div>
                <p className="font-semibold text-surface-800">{preview.patientName}</p>
                <p className="text-sm text-surface-500">{typeIcon[preview.type]} {preview.type} • {preview.uploadedAt?.split('T')[0]}</p>
                {preview.description && <p className="text-xs text-surface-400 mt-1">{preview.description}</p>}
              </div>
