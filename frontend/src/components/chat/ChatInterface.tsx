  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl border border-surface-100 shadow-card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-100 bg-gradient-to-r from-dental-50 to-mint-50">
        <div className="w-16 h-16 rounded-xl bg-gradient-dental flex items-center justify-center text-white text-4xl">
          🤖
        </div>
        <div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === "assistant" && (
              <div className="w-7 h-7 rounded-lg bg-gradient-dental flex items-center justify-center text-white text-xs flex-shrink-0 mt-1">
                🤖
              </div>
            )}
            <div>
            ...
            {msg.role === "user" && (
              <div className="w-7 h-7 rounded-lg bg-surface-200 flex items-center justify-center text-xs flex-shrink-0 mt-1">
                👤
              </div>
            )}
          </div>
        ...
        {loading && (
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-gradient-dental flex items-center justify-center text-white text-xs">
              🤖
            </div>
            <div className="chat-bubble-ai flex items-center gap-1.5 py-3">
              <div
                className="w-2 h-2 rounded-full bg-dental-400 animate-bounce"
