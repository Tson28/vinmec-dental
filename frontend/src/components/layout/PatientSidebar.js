...
export default function PatientSidebar() {
    ...
    const menuItems = [
        ...
        { icon: "👤", label: "Hồ sơ", path: "/patient/profile" },
        { icon: "📅", label: "Lịch hẹn", path: "/patient/appointments" },
        { icon: "📋", label: "Hồ sơ bệnh án", path: "/patient/records" },
    ...
    return (_jsxs("div", { className: `sidebar transition-all duration-300 ${isOpen ? "w-64" : "w-20"} bg-white text-gray-900 h-screen fixed left-0 top-0 flex flex-col border-r border-gray-200`, children: [_jsxs("div", { className: "p-6 border-b border-gray-200 flex items-center justify-between hover:bg-gray-50 transition-colors", children: [_jsxs("div", { className: `flex items-center gap-3 ${!isOpen && "hidden"}`, children: [_jsx("div", { className: "w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg flex items-center justify-center font-bold text-white shadow-lg", children: "P" }), _jsxs("div", { children: [_jsx("span", { className: "font-bold text-lg block text-gray-900", children: "Patient" }), _jsx("span", { className: "text-xs text-gray-500", children: "Portal" })] })] }), isOpen && (_jsx("button", { onClick: () => setIsOpen(!isOpen), className: "hover:bg-gray-200 p-2 rounded transition-colors text-gray-600", title: "Collapse", children: "\u2039" })), !isOpen && (_jsx("button", { onClick: () => setIsOpen(!isOpen), className: "hover:bg-gray-200 p-2 rounded transition-colors w-full text-gray-600", title: "Expand", children: "\u203A" }))] }), _jsx("nav", { className: "flex-1 overflow-y-auto py-6", children: _jsxs("div", { className: `space-y-1 ${isOpen ? "px-4" : "px-2"}`, children: [isOpen && (_jsx("div", { className: "text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4 px-3", children: "Danh m\u1EE5c" })), menuItems.map((item) => (_jsxs("button", { onClick: () => navigate(item.path), className: `w-full flex items-center gap-3 px-3 py-3 rounded-lg transition-all duration-200 ${isActive(item.path)
